...
export function SidebarViewport(
  ...
  const { contentClassName, showGradients = true, ...rest } = props;
  return (
    <ScrollArea
      showScrollbar={false}
      {...rest}
      className={cn("min-h-0 flex-1 relative", rest.className)}
    >
      <ScrollViewport
        className={cn(
          "p-12 pb-12 pt-16 overscroll-contain max-h-[calc(100vh-10rem)]",
          contentClassName,
        )}
      >
        {props.children}
